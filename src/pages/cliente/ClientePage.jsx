...
function buildModal(type, detail, showToast) {
  const m = {
    alerta:{title:`Alerta`,body:`<div style="padding:10px;border-radius:8px;background:#FFF5F5;border:1px solid rgba(224,48,48,.2);margin-bottom:12px;font-size:.75rem;font-weight:700;color:#e03030">${detail}</div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA recomienda actuar en las próximas 24h.</div>`,actions:[{label:'Gestionar',type:'primary',fn:()=>showToast('✅ Alerta gestionada')},{label:'Cerrar',type:'gray'}]},
    ...
    incidencia:{title:`Gestionar incidencia: ${detail}`,body:`<div style="padding:10px;border-radius:8px;background:#FFF5F5;border:1px solid rgba(224,48,48,.2);margin-bottom:12px"><div style="font-size:.72rem;font-weight:700;color:#e03030;margin-bottom:4px">Retraso 48h detectado</div><div style="font-size:.65rem;color:#3a4a5a">Grasas Industriales notificó incidencia logística. ETA original no cumplida.</div></div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA puede buscar alternativas de entrega urgente o negociar compensación.</div>`,actions:[{label:'Buscar alternativa',type:'primary',fn:()=>showToast('🧠 IA buscando alternativa...')},{label:'Reclamar',type:'blue',fn:()=>showToast('📩 Reclamación enviada')},{label:'Cerrar',type:'gray'}]},
    comparativa:{title:`Comparativa IA: ${detail}`,body:`<table style="width:100%;border-collapse:collapse;font-size:.68rem;margin-bottom:12px"><thead><tr style="background:#1A2F4A"><th style="padding:8px;text-align:left;color:rgba(255,255,255,.6)">Proveedor</th><th style="padding:8px;text-align:center;color:rgba(255,255,255,.6)">Precio</th><th style="padding:8px;text-align:center;color:rgba(255,255,255,.6)">Score</th></tr></thead><tbody><tr style="background:#FFF8F0;border-left:3px solid #E87420"><td style="padding:8px;font-weight:700">Grasas Industriales ⭐</td><td style="padding:8px;text-align:center;color:#2D8A30;font-weight:700">2,48€</td><td style="padding:8px;text-align:center;color:#E87420;font-weight:700">94</td></tr><tr><td style="padding:8px">MargaPro Europe</td><td style="padding:8px;text-align:center">2,62€</td><td style="padding:8px;text-align:center;color:#1A78FF;font-weight:700">87</td></tr></tbody></table><div style="padding:10px;background:rgba(232,116,32,.06);border-radius:8px;font-size:.65rem">IA recomienda Grasas Industriales. Ahorro anual: <strong>14.880€</strong></div>`,actions:[{label:'Solicitar pedido',type:'primary',fn:()=>showToast('✅ Pedido solicitado')},{label:'Negociar',type:'blue',fn:()=>showToast('💬 Negociación iniciada')},{label:'Cerrar',type:'gray'}]},
    negociar:{title:`Negociar: ${detail}`,body:`<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px"><div style="display:flex;justify-content:space-between;padding:8px;border-radius:8px;background:#F8FAFC"><span style="font-size:.68rem">Precio actual</span><span style="font-size:.68rem;font-weight:700">2,62€/kg</span></div><div style="display:flex;justify-content:space-between;padding:8px;border-radius:8px;background:rgba(45,138,48,.06)"><span style="font-size:.68rem">Objetivo IA</span><span style="font-size:.68rem;font-weight:700;color:#2D8A30">2,48€/kg (-5,3%)</span></div></div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA enviará propuesta al proveedor automáticamente.</div>`,actions:[{label:'Enviar propuesta',type:'primary',fn:()=>showToast('✅ Propuesta enviada')},{label:'Cancelar',type:'gray'}]},
    ...
    contactar:{title:`Contactar: ${detail}`,body:`<div style="font-size:.72rem;color:#3a4a5a;margin-bottom:12px">Contactar con: <strong>${detail}</strong></div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA redacta el mensaje automáticamente según el contexto.</div>`,actions:[{label:'Llamar',type:'green',fn:()=>showToast('📞 Llamando...')},{label:'WhatsApp',type:'blue',fn:()=>showToast('💬 WhatsApp abierto')},{label:'Email IA',type:'primary',fn:()=>showToast('✉️ Email enviado')},{label:'Cerrar',type:'gray'}]},
    ...
    cotizar:{title:`Cotizar: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA enviará solicitud de cotización a todos los proveedores verificados con <strong>${detail}</strong>.</div>`,actions:[{label:'Solicitar cotización',type:'primary',fn:()=>showToast('🧠 Cotización solicitada')},{label:'Cancelar',type:'gray'}]},
    ...
    comunicar:{title:`Mensaje: ${detail}`,body:`<div style="font-size:.72rem;color:#3a4a5a;margin-bottom:12px">Comunicación con: <strong>${detail}</strong></div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;border-left:3px solid #E87420">IA redacta automáticamente según el contexto.</div>`,actions:[{label:'WhatsApp',type:'green',fn:()=>showToast('💬 WhatsApp abierto')},{label:'Email',type:'blue',fn:()=>showToast('✉️ Email enviado')},{label:'Cerrar',type:'gray'}]},
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        <div className="fab-topbar" style={{ height:56, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 16px', background:'#fff', borderBottom:'1px solid #E8D5C0', boxShadow:'0 1px 12px rgba(26,47,74,.06)', position:'relative', gap:8 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            <span style={{ padding:'3px 10px', borderRadius:20, fontSize:'.62rem', fontWeight:700, letterSpacing:'.06em', textTransform:'uppercase', background:'#EEF5FF', color:'#1A78FF', border:'1px solid rgba(26,120,255,.25)', whiteSpace:'nowrap' }}>Cliente</span>
