      ...
      <Card style={{ marginBottom:13, border:`2px solid rgba(232,116,32,.3)`, background:'linear-gradient(135deg,#FFFBF5,#FFF3E8)' }}>
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'.6rem', minWidth:560 }}>
            ...
            <tbody>
              {[['Azúcar blanco 25kg','0,82€/kg ★','0,89€/kg','0,94€/kg','—','-12,8%'],['Harina T-45 repostera','0,68€/kg ★','0,75€/kg','—','—','-9,3%'],['Tartaletas mini x200','—','0,045€/ud ★','0,052€/ud','—','-13,5%'],['Yema pastelera UHT','3,20€/kg','2,85€/kg ★','—','3,10€/kg','-10,9%'],['Fios D\'ovos 12kg','—','—','—','4,20€/kg ★','Único'],['Marquise chocolate','—','—','5,40€/kg ★','—','Único'],['Vol au Vent grande','—','0,18€/ud','0,15€/ud ★','—','-16,7%']].map(([prod,...cols],i)=>(
                <tr key={i} style={{ borderBottom:'1px solid #F0E4D6' }} onMouseEnter={e=>e.currentTarget.style.background='#FFF8F0'} onMouseLeave={e=>e.currentTarget.style.background=''}>
        ...
        <div className="grid-2">
          ...
          <div style={{ background:'#fff', borderRadius:10, padding:16, border:'1.5px solid rgba(45,138,48,.3)' }}>
            ...
            {[{color:'#2D8A30',label:'CLIENTE',val:'Panaderías Leopold S.L.'},{color:'#1A78FF',label:'CONTACTO',val:'Jefe de compras'},{color:ACCENT,label:'FEEDBACK',val:'Harina W-280 OK. Pide plazo 48h.'},{color:'#e8a010',label:'OPORTUNIDAD',val:'Interés en harinas ecológicas (nueva línea)'}].map((f,i)=>(
              <div key={i} style={{ marginBottom:8, padding:8, background:`rgba(${f.color==='#2D8A30'?'45,138,48':f.color==='#1A78FF'?'26,120,255':f.color===ACCENT?'232,116,32':'232,160,16'},.04)`, borderRadius:6 }}>
        ...
        <div style={{ background:'#fff', borderRadius:18, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(26,47,74,.3)', animation:'modalIn .25s ease both' }}>
          ...
          <div style={{ padding:'20px 22px' }}>
            ...
            <div style={{ background:'linear-gradient(135deg,#FFFBF5,#FFF8F0)', border:'1.5px solid rgba(232,116,32,.25)', borderRadius:12, padding:16, marginBottom:18 }}>
              ...
              <div style={{ marginTop:10, padding:'8px 10px', background:'rgba(45,138,48,.06)', borderRadius:8, fontSize:'.62rem', color:'#2D8A30' }}>
...
function buildModal(type, detail, showToast) {
  const m = {
    alerta:{title:'Alerta',body:`<div style="padding:10px;border-radius:8px;background:#FFF5F5;border:1px solid rgba(224,48,48,.2);margin-bottom:12px;font-size:.75rem;color:#e03030;font-weight:700">${detail}</div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA recomienda actuar en las próximas 24h.</div>`,actions:[{label:'Gestionar',type:'primary',fn:()=>showToast('✅ Alerta gestionada')},{label:'Cerrar',type:'gray'}]},
    checkin:{title:`Check-in: ${detail}`,body:`<div style="padding:12px;background:#F0FFF4;border-radius:8px;margin-bottom:12px"><div style="font-size:.72rem;font-weight:700;color:#2D8A30;margin-bottom:4px">✓ Check-in registrado automáticamente</div><div style="font-size:.62rem;color:#3a4a5a">Cliente: <strong>${detail}</strong><br/>Hora de llegada: ${new Date().toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}<br/>GPS verificado · FoodBridge IA</div></div><div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA ha registrado la visita y actualizado el CRM automáticamente.</div>`,actions:[{label:'Registrar resultado',type:'primary',fn:()=>showToast('✅ Visita registrada')},{label:'Cerrar',type:'gray'}]},
    llamar:{title:`Llamar: ${detail}`,body:`<div style="font-size:.72rem;color:#3a4a5a;margin-bottom:12px">Contactar con: <strong>${detail}</strong></div><div style="padding:10px;background:rgba(45,138,48,.05);border-radius:8px;font-size:.65rem">IA tiene el historial de este cliente listo para la llamada.</div>`,actions:[{label:'Llamar ahora',type:'green',fn:()=>showToast('📞 Llamando...')},{label:'WhatsApp',type:'blue',fn:()=>showToast('💬 WhatsApp abierto')},{label:'Cerrar',type:'gray'}]},
    ...
    cotizar:{title:`Cotizar IA: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem;margin-bottom:12px">IA generará cotización automática comparando todos los fabricantes disponibles para <strong>${detail}</strong>.</div>`,actions:[{label:'Generar cotización IA',type:'primary',fn:()=>showToast('🧠 Cotización generada')},{label:'Cancelar',type:'gray'}]},
    buscar:{title:`Búsqueda IA: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA buscará en las 1.647 fichas técnicas para encontrar más matches de <strong>${detail}</strong>.</div>`,actions:[{label:'Buscar ahora',type:'primary',fn:()=>showToast('🧠 IA buscando...')},{label:'Cancelar',type:'gray'}]},
    ...
    agendar:{title:`Agendar visita: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA añadirá la visita a tu ruta optimizada para la próxima semana disponible.</div>`,actions:[{label:'Añadir a ruta',type:'primary',fn:()=>showToast('📅 Visita añadida a la ruta')},{label:'Cancelar',type:'gray'}]},
    resultado:{title:`Resultado visita`,body:`<div style="font-size:.72rem;color:#3a4a5a;margin-bottom:12px">Registrar resultado: <strong>${detail}</strong></div><div style="padding:10px;background:rgba(45,138,48,.05);border-radius:8px;font-size:.65rem">IA actualizará el CRM y generará el seguimiento automáticamente.</div>`,actions:[{label:'Confirmar',type:'primary',fn:()=>showToast('✅ Resultado registrado')},{label:'Cerrar',type:'gray'}]},
    ...
    sugerir:{title:`Sugerir a clientes: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA enviará sugerencia personalizada a los clientes más adecuados para <strong>${detail}</strong>.</div>`,actions:[{label:'Enviar sugerencia',type:'primary',fn:()=>showToast('✅ Sugerencia enviada')},{label:'Cancelar',type:'gray'}]},
    aplicar:{title:`Aplicar: ${detail}`,body:`<div style="padding:10px;background:rgba(45,138,48,.05);border-radius:8px;font-size:.65rem">IA aplicará los cambios de tarifa y notificará automáticamente a todos los clientes afectados.</div>`,actions:[{label:'Confirmar y aplicar',type:'primary',fn:()=>showToast('✅ Tarifas actualizadas')},{label:'Cancelar',type:'gray'}]},
    enviar:{title:`Enviar: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA personalizará el mensaje para cada cliente según su historial de pedidos.</div>`,actions:[{label:'Enviar ahora',type:'primary',fn:()=>showToast('✅ Tarifa enviada a todos los clientes')},{label:'Cancelar',type:'gray'}]},
    ...
    tarea:{title:`Nueva tarea: ${detail}`,body:`<div style="padding:10px;background:rgba(26,120,255,.05);border-radius:8px;font-size:.65rem">Tarea creada: <strong>${detail}</strong>. IA la añadirá a tu agenda.</div>`,actions:[{label:'Añadir a agenda',type:'primary',fn:()=>showToast('📅 Tarea añadida')},{label:'Cerrar',type:'gray'}]},
    ...
    muestra:{title:`Solicitar muestra: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA solicitará la muestra al fabricante y coordinará la entrega al cliente.</div>`,actions:[{label:'Solicitar',type:'primary',fn:()=>showToast('✅ Muestra solicitada')},{label:'Cancelar',type:'gray'}]},
    ...
    recordar:{title:`Recordatorio: ${detail}`,body:`<div style="padding:10px;background:rgba(232,116,32,.05);border-radius:8px;font-size:.65rem">IA enviará un recordatorio al cliente para que valore la muestra.</div>`,actions:[{label:'Enviar recordatorio',type:'primary',fn:()=>showToast('✅ Recordatorio enviado')},{label:'Cancelar',type:'gray'}]},
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        <div className="fab-topbar" style={{ height:56, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 16px', background:'#fff', borderBottom:'1px solid #E8D5C0', boxShadow:'0 1px 12px rgba(26,47,74,.06)', position:'relative', gap:8 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            <span className="fab-topbar-rolebadge" style={{ padding:'3px 10px', borderRadius:20, fontSize:'.62rem', fontWeight:700, letterSpacing:'.06em', textTransform:'uppercase', background:'#F0FFF4', color:'#2D8A30', border:'1px solid rgba(45,138,48,.25)', whiteSpace:'nowrap' }}>Comercial</span>
